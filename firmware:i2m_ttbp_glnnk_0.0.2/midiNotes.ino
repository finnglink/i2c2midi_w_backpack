...
void sendMidiNoteOn(int channel, int noteNumber, int velocity) {
  ...
  if (isTRS(channel)) {
    ...
    // Send BLE Note On
    blemidi.write((uint8_t)(0x90 | (channel & 0x0F)));  // Note On, actual channel
    blemidi.write(noteNumber);
    blemidi.write(velocity);
...
void sendMidiNoteOff(int channel, int noteNumber) {
  if (isTRS(channel)) {
    ...
    // Send BLE Note Off
    blemidi.write((uint8_t)(0x80 | (channel & 0x0F)));  // Note On, actual channel
    blemidi.write(noteNumber);
